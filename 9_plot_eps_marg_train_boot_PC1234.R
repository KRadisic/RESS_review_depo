...
for (n_pc in c(4))
{
  for (ntraj_num_samples_boot in Ntraj_num_samples_boot){
    ...
    for (n_boot in seq(1:Nboot)){
      ...
      df_pc <- as.data.frame(pca$x)[,seq(1,length(A_multidx[,1]))]
  ...
  setwd(metamodel_folder)
  save(df_eps_marg_train_bootstrap, file = paste('df_eps_marg_train_PC',n_pc,'_bootstrap_beta',str_beta,'.RData', sep=""))
}
...
dfdf_eps_marg_train_bootstrap <- rbind(temp1,temp2, temp3, df_eps_marg_train_bootstrap)
dfdf_eps_marg_train_bootstrap$nPCs <- c(rep('nPC=1', length(temp1$eps_marg)), 
                                        rep('nPC=2', length(temp2$eps_marg)),
                                        rep('nPC=3', length(temp3$eps_marg)),
                                        rep('nPC=4', length(df_eps_marg_train_bootstrap$eps_marg)))
...
gg_eps_marg_train_bootstrap <- ggplot(dfdf_eps_marg_train_bootstrap, aes(x=num_traj, y=eps_marg, color = nPCs,  group = interaction(num_traj,nPCs))) + 
  ...
  scale_color_manual(name = "", values = c('nPC=1' = "#253494", 'nPC=2' = "#2c7fb8", 'nPC=3' = "#41b6c4", 'nPC=4' = "#a1dab4"))+
  ...
  xlab('$R_{train}$, number of train trajectories') + ylab('$\\epsilon_{marg}$') +
