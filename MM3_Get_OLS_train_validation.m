...
% make OLS PCE for each cost function under each rain, where the cost
...
for r = 1:200 % number of cost functions to minimize
    ...
    % fit and save PCE MM
    name_file = strcat('pesh_profmoist_Jpce_errorLogN02_truerain',int2str(true_rain_idx),'_Jb',str_beta,'_Ridx', int2str(r), 'N_train', int2str(N_train));
    cd(data_folder)
...
for r = 1:499 % number of cost functions
    ...
    % fit and save PCE MM
        name_file = strcat('TEST_pesh_profmoist_Jpce_errorLogN02_truerain',int2str(true_rain_idx),'_Jb',str_beta, ...
            '_Ridx', int2str(r), 'N_train', int2str(N_train));
        cd(data_folder)
