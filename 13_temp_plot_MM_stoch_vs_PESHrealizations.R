...
x_new_traj <- npyLoad('LHS_MMtrajectories_LogN02_beta005_condmin174_Nomega50_nlhs4500_dim6_sample_final.npy')
...
x_J_train_rains$value.J <- x_J_train_rains$J0 + Jb.train

head(x_J_train_rains)

## TO DATAFRAME FOR PLOTTING
x_J_train_rains$parameter.name  <- rep(rep(names_params, each = n_lhs_plot), Nomega)
x_J_train_rains$parameter.name <- factor(x_J_train_rains$parameter.name, 
                                      levels = c("$\\theta_{s.inter}$",
                                                 "$mn_{deep}$","$\\theta_{s.deep}$","$\\theta_{s.surf}$", "$\\theta_{r.deep}$"))# levels = ASI_order)

idxs_cols <- rep(seq(1,5),Nomega*n_lhs_plot)
idxs_rows_begin <- seq(1, by = 60, to = Nomega * 5 * 60)
idxs_rows_end <- seq(60, by = 60, to = Nomega * 5 * 60)
only_varying_param <- c()
for (idx in seq(1,15000/60))
{
  only_varying_param <- c(only_varying_param, x_J_train_rains[idxs_rows_begin[idx]:idxs_rows_end[idx], idxs_cols[idx]])
}

x_J_train_rains$variable  <- only_varying_param
x_J_train_rains$rain.idx  <- as.factor(rep(seq(1,50),each = 5*n_lhs_plot))
head(x_J_train_rains)
...
# louis, peshmelba
gg <- ggplot() + 
  geom_line(data = x_J_train_rains, aes(x = variable, y = value.J, group = rain.idx), alpha = 0.7) + theme_bw() +
  facet_wrap(vars(parameter.name), scales = "free_x", ncol = 3) + xlab('') + ylab('') +
  geom_hline(yintercept = 0.01, color="#386cb0",linetype="solid",linewidth = 1.0)+
  geom_hline(yintercept = 0.02, color="#f0027f",linetype="solid",linewidth = 1.0)+
  theme(legend.position = "bottom")   
...
# ONE OVER THE OTHER
df_mm_vs_pesh_1 <- x_J_train_rains[,8:11]
df_mm_vs_pesh_1$type <- 'PESHMELBA,  $f_s$'
df_mm_vs_pesh_2 <- data.frame(df_plot[,c(4,1,3,2)])
df_mm_vs_pesh_2$type <- 'stoch. emulator, $\\hat{f}_s$'
head(df_mm_vs_pesh_1)
head(df_mm_vs_pesh_2)
df_mm_vs_pesh <- rbind(df_mm_vs_pesh_1, df_mm_vs_pesh_2)
head(df_mm_vs_pesh)

gg_both <- ggplot(data = df_mm_vs_pesh, aes(x = variable, y = value.J, group = interaction(rain.idx,type), color = type)) + 
  geom_line(alpha = 1) + theme_bw() +
  facet_wrap(vars(parameter.name), scales = "free_x", ncol = 3) + xlab('') + ylab('') +
  theme(legend.position = "bottom")   
gg_both
...
tikz(paste('MM_stoch_vs_PESH_beta',str_beta,'_moistprof_fix_',fixed_point_name,'.tex', sep = ''), standAlone = TRUE, width=5.5, height=4.0)
gg_both
dev.off()
tools::texi2dvi(paste('MM_stoch_vs_PESH_beta',str_beta,'_moistprof_fix_',fixed_point_name,'.tex', sep = ''),pdf=T)
