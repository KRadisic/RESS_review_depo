...
#######################################################
Nboot <- 50
Ntraj_original_sample <- 1:200 # number of trajectories
Ntraj_num_samples_boot <- c(50,100,150,200)
NX_test_set <- 1:100

vect_params <- 1:5
df_eps_marg_train_bootstrap <- c()

for (ntraj_num_samples_boot in Ntraj_num_samples_boot){
  eps_marg_train_traj_bootstrap <- c()
  print(paste('ntraj_num_samples_boot = ', ntraj_num_samples_boot))
  ...
  for (n_boot in seq(1:Nboot)){
    ...
    ##  bootstrapping the training trajectories
    boot_trajectories1 <- sample(Ntraj_original_sample, size = ntraj_num_samples_boot, replace = TRUE)
    
    ###     PRINCIPAL COMPONENT ANALYSIS
    train_rain_idx <- boot_trajectories1 #c(1:81,83:200)  # Define training data indices
    ...
    df_latent_trainvsKDE <- rbind(df_new_points,df_pca[,1:n_pc])
  ...
  df_temp <- data.frame(eps_marg = eps_marg_train_traj_bootstrap, 
                        num_traj = rep(ntraj_num_samples_boot, Nboot))
  df_eps_marg_train_bootstrap <- rbind(df_eps_marg_train_bootstrap, df_temp)
}# close loop over all trajectories numbers

setwd(metamodel_folder)
save(df_eps_marg_train_bootstrap, file = paste('df_eps_marg_train_bootstrap_beta',str_beta,'.RData'))
...
setwd(metamodel_folder)
load('eps_marg_valid_bootstrap.RData')
...
ggplot(df_eps_marg_train_bootstrap, aes(x=num_traj, y=eps_marg, group = num_traj)) + geom_boxplot(outlier.colour=NA)  + theme_bw() + 
  geom_rect(aes(ymin=min(quantile_05), ymax=max(quantile_95), xmin=-Inf, xmax=Inf), alpha=0.002) + 
  geom_rect(aes(ymin=min(quantile_25), ymax=max(quantile_75), xmin=-Inf, xmax=Inf), alpha=0.10) + 
  xlab('Number of trajectories') + ylab('$\\eps\\_marg$') +
  geom_hline(yintercept = mean(eps_marg_valid_bootstrap), color = 'black', linetype = 'dashed', linewidth = 1) # this bootstrap is only calculated for beta = 0.005
