...
#{
  ...
  #for (Nx_valid in Nx_num_samples_boot){
    ...
    ml_coeff_multidx_r_TEST_boot <- read.csv(paste("ml_boot_multicoeff_c_alpha_OLS_of_R500_TEST5OOboot_pesh_profmoist_Jpce_errorLogN02_truerain53_Jb",
                                                   str_beta,"_of20Nx_valid100.csv", sep=""), header = FALSE)
    ...
    for (n_boot in seq(1:Nboot)){
      ...
      for (idx_test_x_param in seq(1,nb_x_param)){
        ...
        ## save figures
        setwd(paste(working_directory, "/output_figures_bootstrap_Nx", sep=""))
  ...
  setwd(metamodel_folder)
  save(eps_marg_train_traj_bootstrap, file = paste('eps_marg_valid_bootstrap_Nx',100,'.RData', sep=""))
#}
